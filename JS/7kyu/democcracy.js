...
function trilingualDemocracy(group) {
    ...
    if (Object.values(counts).some(count => count === 2) && Object.keys(counts).length === 2) {
        return Object.keys(counts).find(language => counts[language] === 1);
    }

    const languages = ['D', 'F', 'I', 'K'];
    return languages.filter(language => !counts[language])[0];
}